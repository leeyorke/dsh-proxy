...
>
> The standalone (Go/Node single-file, no DSH required) lives at <https://github.com/leeyorke/dsh-proxy>; this is its integrated sibling with the same native-Basic-Auth model.
...
The installed package is **`@leeyorke/dsh-proxy`** (scoped, with the username prefix) and shows up as `@leeyorke/dsh-proxy@0.1.1` in `dsh plugin ls`.
...
- **Harness browser session**: DSH 0.1.6-alpha+ requires a browser-session cookie for the index and `/api`. The proxy performs the one-time launch-token exchange on the entry navigation (the upstream answers 303 + Set-Cookie; the token never reaches the client). The injection happens **once per browser session** — an entry mark keeps the harness's 303 from becoming a redirect loop, and a dead harness session re-runs the exchange on the next navigation. Basic Auth remains the single gate for the whole LAN surface — the session is obtained behind it. The settings page's `/dsh-proxy` channel requires the same session, which proxied visitors carry as a cookie.
