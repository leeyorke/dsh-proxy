...
- `src/` — TypeScript source. `src/proxy.ts` is the pure-node proxy core (native Basic Auth gate, HTTP + WebSocket forwarding, Host/Origin alignment, polyfill injection, launch-token entry injection for the harness's browser-session login) with **no cordis dependency**; `src/controller.ts` is the proxy controller (effective options = cordis base + persisted overlay, start/stop/restart, status/update/start/stop verbs, persistence to `$DSH_HOME/dsh-proxy.json`); `src/rpc-route.ts` is the `/dsh-proxy` channel's own host route and Connection-envelope bridge (pure `node:http`, no cordis import — the plugin registers it on `ctx.webServer` itself instead of going through `connection.rpc.handle`, whose internal `webServer` read breaks on current harnesses); `src/index.ts` is the cordis plugin entry (Config schema, effect lifecycle, channel wiring, launch-token acquisition via `connection.authenticatedUrl`); `src/session.ts` / `src/settings.ts` / `src/contract.ts` are pure, unit-tested helpers and the shared wire contract; `src/polyfill.ts` is the randomUUID shim; `src/client/` is the browser settings section (locales, styles, React component, plugin body). There is deliberately **no login page and no session cookie** — authentication is HTTP Basic Auth only (the browser's native dialog), matching the standalone dsh-proxy.
...
- This plugin is named `dsh-proxy` (matching the repository) and its source lives at `dsh-lan-proxy/` inside the dsh-proxy repository (the DSH-plugin variant, alongside the standalone `go/` and `node/` builds). The repo ROOT carries a thin package manifest (name `dsh-proxy`) so the plugin installs from the repository itself: online `dsh plugin --profile <name> add github:leeyorke/dsh-proxy#master` (recommended) or local `dsh plugin --profile <name> add file:C:/mydata/codes/dsh-proxy` (bundle route); changes take effect on the **next `dsh web` restart**.
- Dev type sources come from the npm registry, pinned to the versions the current harness (`@deepseek-ai/dsh@0.1.6-alpha.2`) ships internally: `@deepseek-ai/cordis@^4.0.2`, `@deepseek-ai/schemastery@^3.18.2`, and the `@deepseek-ai/dsh-*` packages at exact `0.1.6-alpha.2` (exception: `@deepseek-ai/dsh-client-runtime` has no 0.1.6-alpha.2 publication — it is pinned to `0.1.1-rc.2`, the newest published; the client half only uses `ClientContext`'s stable surface). When the harness version moves, bump these to the new harness's bundled versions and re-run `pnpm install` to fix API drift.
- **Why the channel route is self-owned:** `connection.rpc.handle(...)` used to be the mount path, but the harness's dedicated-channel implementation reads `webServer` through the client-connection plugin's fiber (a cordis traceable-shadow walk). That worked only while client-connection statically injected `webServer`; since the harness made `webServer` optional there (`fix: windows build`, 2026-08-31, in the 0.1.6-alpha line), the walk throws `cannot get property "webServer" without inject` for every external plugin. The plugin therefore registers its route on the injected `ctx.webServer` directly (the same pattern the harness's own api-gateway uses for its upgrade route) and fences it with `ctx.connection.requestRejection` (Host/Origin trust + browser-session auth, identical policy to `/api`). Do not migrate back to `rpc.handle` without re-verifying against the harness version in use.
